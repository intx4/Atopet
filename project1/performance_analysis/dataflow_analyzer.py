...
for path, var in zip(paths, vars):
    ...
    if path == 'num_party_change' or path == 'secrets_multiplications':
        fig1,(ax1, ax3) = plt.subplots(2)
        ax1.set(title='Bytes Received')
        ...
        for ax in fig1.get_axes():
            ax.label_outer()
        plt.savefig('dataflow_in_'+path+'.png')
        fig2,(ax2, ax4) = plt.subplots(2)
        ax2.set(title='Bytes Sent')
        ax2.set(ylabel='avg dataflow in bytes')
        ax2.errorbar(x, m_out, yerr=e_out, fmt='--o', color='orange', ecolor='lightgray', elinewidth=3)
        ax4.set(xlabel=var)
        ax4.set(xlabel=var, ylabel='variance')
        ax4.plot(x, v_out, 'o', color='red')
        for ax in fig2.get_axes():
            ax.label_outer()
        plt.savefig('dataflow_out_'+path+'.png')
    else:
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2)
        ax1.set(ylabel='avg dataflow in bytes')
        ax1.set(title='Byte Received')
        ax1.errorbar(x, m_in, yerr=e_in, fmt='--o', color='blue', ecolor='lightgray', elinewidth=3)
        ax2.errorbar(x, m_out, yerr=e_out, fmt='--o', color='orange', ecolor='lightgray', elinewidth=3)
        ax2.set(title='Byte Sent')
        ax3.set(xlabel=var, ylabel='variance')
        ax3.plot(x, v_in, 'o', color='green')
        ax4.set(xlabel=var)
        ax4.plot(x, v_out, 'o', color='red')
