...
def split_secret_in_shares(secret: int, total_num_shares: int, is_secret: bool) -> List[Share]:
    ...
    for _ in range(0, total_num_shares - 1):
        ...
        shares.append(Share(rand, is_secret))
    s0 = (secret - sum(shares)) % shares[0].FIELD
    shares.append(Share(s0, is_secret))
    return shares
